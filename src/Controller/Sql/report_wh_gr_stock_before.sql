select sum(gl.qty) as totalqty,pc.label,p.percent,sdw.code,pc.id as product_category_id   
from 
...
where g.type = 'GR' and g.status !='VO' and g.to_warehouse = :warehouse_id and (g.docdate < :startdate)
