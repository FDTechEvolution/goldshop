...
class ExcelComponent extends Component {
    ...
    public function readToArray($fullFilePath) {
        ...
        foreach ($objPHPExcel->getWorksheetIterator() as $worksheet) {
            ...
            $headings = $worksheet->rangeToArray('A1:' . $highestColumn . 1, NULL, false, true);

            $_row = [];
            $countColumn = 0;
            for ($row = 1; $row <= $highestRow; $row++) {

                $rowData = $worksheet->rangeToArray('A' . $row . ':' . $highestColumn . $row, NULL, false, true);
