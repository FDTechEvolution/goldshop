...
class UpdatedataController extends AppController {
    ...
    public function goldprice() {
        //$content = file_get_contents('https://www.goldtraders.or.th/default.aspx');
        $content = $this->urlOpener('https://www.goldtraders.or.th/default.aspx');
    ...
    private function sendDailyPriceToLine($price) {
        ...
        $str = $str . chr(10) . 'ทองคำแท่ง';
        $str = $str . chr(10) . 'รับซื้อ ' . (Number::format($price->gold_bar_purchase_price)) . ', ขายออก ' . (Number::format($price->gold_bar_sales_price));
        $str = $str . chr(10) . 'ทองรูปพรรณ';
        $str = $str . chr(10) . 'รับซื้อ ' . (Number::format($price->gold_purchase_price)) . ', ขายออก ' . (Number::format($price->gold_sales_price));
